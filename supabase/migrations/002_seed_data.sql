...
-- Insert sample equipment (without cpu column for now, will be updated later)
INSERT INTO equipment (name, type, brand, model, serial_number, asset_number, department_id, current_user_id, status, warranty_date, purchase_date, purchase_price, supplier, ram, storage, operating_system, ip_address, mac_address, hostname, location, notes) VALUES
('คอมพิวเตอร์ตั้งโต๊ะ Dell OptiPlex 7090', 'computer', 'Dell', 'OptiPlex 7090', '7XKWN3J', 'PC001', (SELECT id FROM departments WHERE code = 'IT'), (SELECT id FROM users WHERE username = 'manager1'), 'normal', '2025-12-31', '2023-01-15', 45000.00, 'Dell Thailand', '8GB DDR4', '256GB SSD', 'Windows 11 Pro', '192.168.1.100', '00:1B:44:11:3A:B7', 'IT-PC001', 'ห้อง 201 อาคาร A', 'คอมพิวเตอร์สำหรับงานพัฒนา'),
('โน้ตบุ๊ค Lenovo ThinkPad E14', 'laptop', 'Lenovo', 'ThinkPad E14', 'PC1A2B3C', 'NB001', (SELECT id FROM departments WHERE code = 'HR'), (SELECT id FROM users WHERE username = 'user1'), 'normal', '2024-08-15', '2022-06-20', 35000.00, 'Lenovo Thailand', '16GB DDR4', '512GB SSD', 'Windows 11 Home', '192.168.1.101', '00:1B:44:11:3A:B8', 'HR-LAP001', 'ห้อง 301 อาคาร B', 'โน้ตบุ๊คสำหรับงานบุคคล'),
('เครื่องพิมพ์ HP LaserJet Pro 404dn', 'printer', 'HP', 'LaserJet Pro 404dn', 'VNC8K12345', 'PR001', (SELECT id FROM departments WHERE code = 'ACC'), (SELECT id FROM users WHERE username = 'user2'), 'maintenance', '2024-03-20', '2021-09-10', 15000.00, 'HP Thailand', NULL, NULL, NULL, '192.168.1.102', '00:1B:44:11:3A:B9', 'ACC-PRT001', 'ห้อง 401 อาคาร C', 'เครื่องพิมพ์สำหรับงานบัญชี'),
('จอภาพ Samsung 24นิ้ว', 'monitor', 'Samsung', 'S24F350FH', 'SM24F350', 'MN001', (SELECT id FROM departments WHERE code = 'ADMIN'), (SELECT id FROM users WHERE username = 'user3'), 'damaged', '2023-11-10', '2020-12-05', 8000.00, 'Samsung Thailand', NULL, NULL, NULL, NULL, NULL, NULL, 'ห้อง 501 อาคาร D', 'จอภาพสำรอง'),
('คอมพิวเตอร์ตั้งโต๊ะ HP EliteDesk 800 G8', 'computer', 'HP', 'EliteDesk 800 G8', 'HP800G8001', 'PC002', (SELECT id FROM departments WHERE code = 'IT'), NULL, 'normal', '2026-01-15', '2023-03-20', 42000.00, 'HP Thailand', '16GB DDR4', '512GB SSD', 'Windows 11 Pro', '192.168.1.103', '00:1B:44:11:3A:BA', 'IT-PC002', 'ห้อง 202 อาคาร A', 'คอมพิวเตอร์สำหรับงานกราฟิก');
