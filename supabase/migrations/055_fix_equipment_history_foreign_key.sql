-- Migration: 055_fix_equipment_history_foreign_key.sql
-- Description: Fix equipment_history foreign key constraint and trigger function
-- Date: 2024-12-19
-- Author: Assistant
...
-- Update the log_equipment_changes function to handle auth.uid() properly
...
DECLARE
  ...
  new_val TEXT;
BEGIN
  -- Get current user ID - handle both authenticated and anonymous users
  BEGIN
    current_user_id := auth.uid();
  EXCEPTION
    WHEN OTHERS THEN
      current_user_id := NULL; -- Set to NULL if auth.uid() fails
  END;
...
-- Ensure the foreign key constraint has a proper name
DO $$
BEGIN
  -- Check if the constraint exists and rename it if needed
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'equipment_history_changed_by_fkey' 
    AND table_name = 'equipment_history'
  ) THEN
