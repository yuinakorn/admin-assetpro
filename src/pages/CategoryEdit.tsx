...
import { EquipmentCategoryUpdate } from "@/types/database"
import { DashboardLayout } from "@/components/layout/DashboardLayout"

export default function CategoryEdit() {
  const { id } = useParams<{ id: string }>()
  const [formData, setFormData] = useState<EquipmentCategoryUpdate>({
    name: "",
    code: "",
    description: "",
    icon: "Package",
  ...
  const fetchCategory = async (categoryId: string) => {
    try {
      ...
      const category = await EquipmentCategoryService.getCategoryById(categoryId)
      if (category) {
        setFormData({
          name: category.name,
          code: category.code,
        ...
        toast({
          title: "ไม่พบข้อมูล",
          description: "ไม่พบประเภทครุภัณฑ์ที่ต้องการแก้ไข",
          variant: "destructive",
        })
        navigate("/categories")
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const codeExists = await EquipmentCategoryService.checkCodeExists(formData.code, id)
      if (codeExists) {
        toast({
          title: "รหัสซ้ำ",
      ...
      await EquipmentCategoryService.updateCategory(id, formData)
      
      toast({
        title: "แก้ไขสำเร็จ",
        description: "แก้ไขประเภทครุภัณฑ์เรียบร้อยแล้ว",
      })
      ...
      toast({
        title: "เกิดข้อผิดพลาด",
        description: "ไม่สามารถแก้ไขประเภทครุภัณฑ์ได้",
        variant: "destructive",
      })
    } finally {
      setLoading(false)
  ...
  const handleInputChange = (field: keyof EquipmentCategoryUpdate, value: any) => {
    setFormData(prev => ({
      ...prev,
      [field]: value
    }))
      ...
      <DashboardLayout>
        <div className="space-y-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-bold text-foreground">แก้ไขประเภทครุภัณฑ์</h1>
              <p className="text-muted-foreground">
                แก้ไขข้อมูลประเภทครุภัณฑ์
              </p>
            </div>
          </div>
