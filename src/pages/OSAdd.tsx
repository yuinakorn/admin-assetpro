...
export default function OSAdd() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      await osService.createOS({ os_name: osName })
      toast({ title: 'เพิ่มระบบปฏิบัติการสำเร็จ' })
      navigate('/properties/os')
    } catch (error) {
      console.error('Error creating OS:', error)
      toast({ title: 'เกิดข้อผิดพลาดในการเพิ่มระบบปฏิบัติการ', variant: 'destructive' })
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <DashboardLayout>
      <div className="flex items-center mb-4">
        ...
        <h1 className="text-2xl font-bold ml-2">เพิ่มระบบปฏิบัติการใหม่</h1>
      </div>
      <Card>
        <CardHeader>
